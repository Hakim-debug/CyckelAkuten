...
class UserLoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "Bicycle Assistance"
...
//Here you chosse User or Business
    private fun updateUI() {
        ...
        val currentUserDb = uid?.let {
            db.collection("Users").document(it)
        }
        currentUserDb?.get()?.addOnSuccessListener { document ->
            if (document != null) {
                println("Doc: ${document.data}")
                isBusiness = document.data?.get("isBusiness") as Boolean

                if (isBusiness) {
                    val intent = Intent(this, UserRecyclerview::class.java)
